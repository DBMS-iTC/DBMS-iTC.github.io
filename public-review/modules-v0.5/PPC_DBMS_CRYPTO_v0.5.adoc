...
|0.4
|2026-07-20
|Aligned the standalone Crypto composition with the role-specific ML-KEM design. The ST separates TOE KeyGen, encapsulation, and decapsulation; derives Master Keys or KEKs through `FCS_CKM.5`; uses KeyWrap or AEAD for transferred existing keys; identifies operations allocated outside the TOE; and leaves protocol-level ML-KEM to the applicable Functional Package.

|0.5
|2026-07-20
|Updated the composition to the Version 0.5 Cryptographic Functions Module and SD, including the explicit Master Key protection model and normative cryptographic audit-event refinement.
...
This PP-Configuration combines the {base-pp} ({base-pp-version}) with the collaborative PP-Module for DBMS Cryptographic Functions, Version 0.5.
...
* DBMS master key and data encryption key lifecycle requirements, including the optional "Established through Key Encapsulation" origin
...
When non-protocol ML-KEM is selected, the ST shall identify whether the TOE performs KeyGen, encapsulation, decapsulation, or a combination and shall claim only the operations within the TOE boundary. A KEM-established shared secret is input to `FCS_CKM.5` for Master Key or KEK derivation. A transferred pre-existing key remains protected by `FCS_COP.1/KeyWrap` or `FCS_COP.1/AEAD`; it is not described as directly encapsulated by ML-KEM. Protocol-level ML-KEM, including hybrid TLS, remains governed by the applicable Functional Package.
...
The legacy TD_DBMS_B_001 and TD_DBMS_B_002 resolutions are incorporated into the Version 2.0 Base cPP and Supporting Document set and do not apply separately to this PP-Configuration. This PP-Configuration does not introduce additional Technical Decisions.
