...
|0.4
|2026-07-20
|Aligned composition guidance with the Crypto Module's role-specific ML-KEM design. The ST separates TOE KeyGen, encapsulation, and decapsulation from cloud KMS operations outside the TOE; derives Master Keys or KEKs through the selected KDF; uses KeyWrap or AEAD for transferred existing keys; and leaves protocol-level ML-KEM to the applicable Functional Package.

|0.5
|2026-07-20
|Updated the composition to the Version 0.5 Cloud and Cryptographic Functions Modules and SDs, including complete Cloud EA-to-CEM traceability and the explicit Master Key protection model.
...
This PP-Configuration combines the {base-pp} ({base-pp-version}), the collaborative PP-Module for DBMS in the Cloud, Version 0.5, and the collaborative PP-Module for DBMS Cryptographic Functions, Version 0.5.
...
* DBMS cryptographic key management, including role-specific non-protocol ML-KEM operations and the boundary between TOE operations and cloud KMS functionality when those selections are claimed
* Catalogue-derived components consumed in the Crypto Module and applicable Functional Package components

When ML-KEM is selected, the ST shall identify whether the TOE performs KeyGen, encapsulation, decapsulation, or a combination; claim only the operations within the TOE boundary; and map any cloud KMS operation to the operational environment. A KEM-established shared secret is input to `FCS_CKM.5` for Master Key or KEK derivation. A transferred pre-existing key remains protected by `FCS_COP.1/KeyWrap` or `FCS_COP.1/AEAD`; it is not described as directly encapsulated by ML-KEM. Protocol-level ML-KEM, including hybrid TLS, remains governed by the applicable Functional Package.
...
* [#DBMS_Cloud_MOD]#[DBMS_Cloud_MOD]# collaborative PP-Module for DBMS in the Cloud, Version 0.5, 2026-07-20.
* [#DBMS_Cloud_SD]#[DBMS_Cloud_SD]# Supporting Document - Evaluation Activities for DBMS in the Cloud Module, Version 0.5, 2026-07-20.
