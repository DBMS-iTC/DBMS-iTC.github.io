...
. The evaluator does not attempt to establish that infrastructure-level suppression of audit generation by a Trusted Platform administrator is prevented; that residual is held by `A.TRUSTED_PLATFORM_ADMIN` and is not evaluated as TOE behaviour.
...
. Verify that the selected attempts are denied and that the complete inventory identifies no documented interface that performs the disable.
. The evaluator does not attempt to establish that infrastructure-level suppression of data-at-rest encryption by a Trusted Platform administrator is prevented; that residual is held by `A.TRUSTED_PLATFORM_ADMIN` and is not evaluated as TOE behaviour.
